...
fn cancel_returns_the_tokens_to_the_maker() {
    ...
    advance_clock(&mut svm, escrow::constants::CANCEL_DELAY_SECONDS);
...
fn cancel_fails_before_the_time_lock_expires() {
    ...
    // Advance to one second before cancellation becomes available.
    advance_clock(&mut svm, escrow::constants::CANCEL_DELAY_SECONDS - 1);
    ...
    let failure = result.expect_err("cancel should fail before the time lock expires");
    ...
    // A failed cancellation must not move or destroy anything.
    assert_eq!(
        token_amount(&svm, &maker_ata_a),
        0,
        "maker should not receive tokens before the lock expires"
    );
    ...
    assert!(
        svm.get_account(&vault_a).is_some(),
        "vault should remain open after failed cancellation"
    );

    assert!(
        svm.get_account(&escrow_pda).is_some(),
        "escrow should remain open after failed cancellation"
    );
}
