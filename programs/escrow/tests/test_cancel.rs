...
fn test_cancel_too_early() {
    ...
    assert!(res.is_err(), "cancel should have failed but succeeded");
    let err_str = format!("{:?}", res.unwrap_err());
    assert!(err_str.contains("1771") || err_str.contains("TimeLockActive") || err_str.contains("custom program error"), "Expected custom program error, got: {}", err_str);
}

#[test]
fn test_cancel_after_lock() {
    let mut svm = setup_svm();
    let (maker, escrow_pda, mint_a, maker_ata_a, vault_a) = setup_escrow(&mut svm);
    ...
    assert!(res.is_ok(), "cancel after lock failed: {:?}", res.err());
}

#[test]
fn test_cancel_exactly_at_boundary() {
    let mut svm = setup_svm();
    let (maker, escrow_pda, mint_a, maker_ata_a, vault_a) = setup_escrow(&mut svm);
    ...
    assert!(res.is_ok(), "cancel exactly at boundary failed: {:?}", res.err());
}